...
export default function Home({ data }) {
  ...
  const addTodo = (id = NULL) => {
    ...
    // added key to the div
    div.innerHTML = `<div className="card-body" key={getLastId} id={getLastId.toString()}> 
      <h1 className="card-title">${
        newtitle.charAt(0).toUpperCase() + newtitle.slice(1)
      }</h1>
      <p className="card-text">">${
        newBody.charAt(0).toUpperCase() + newBody.slice(1)
      }</p>
      <span>
    ...
    if (id !== NULL) {
      ...
      // setting newBody and new Title as blank
      setNewBody("")
      setNewtitle("")
  ...
  React.useEffect(() => {
    // fixed check on title and body
    var checkTitle = initialData.filters((data) =>
      data.title === newtitle
    );
    var checkBody = initialData.filters((data) => data.body === newBody);
    ...
    setTimeout(() => {
      ...
      // checkBody fixed
      if (checkBody.body.length > 0) {
        alert("Body is already exist");
      }
      // checkTitle & checkBody fixed
      if (checkTitle.title.length > 0 && checkBody.body.length > 0) {
        alert("Title and Body is already exist");
      }
    }, 400);
    //no need for adding initial data as Dependency
  }, [newtitle, newBody]);
    ...
    <React.Fragment>
      <div className="container">
        ...
        {initialData.map((item) => {
            ...
            <div className={`card ${findStatus(item.id) === "danger"?"bg-danger":findStatus(item.id) === "warning"?"bg-warning":"bg-success"}`} key={item.id} id={item.id}>
